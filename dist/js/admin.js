...
var core = __webpack_require__(/*! ../../internals/path */ "./node_modules/core-js-pure/internals/path.js"); // eslint-disable-next-line es/no-json -- safe
...
var WbPo = window.WbPo || {};
/**
 * Set the selected the items in fields.
 *
 * @since 1.0.0
 */
...
function setSelectedItems() {
  ...
  if (document.querySelectorAll('#wbpo_selected li').length) {
    var _context;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context = document.querySelectorAll('#wbpo_selected li')).call(_context, function (item) {
      var _context2, _context3, _context4, _context5, _context6;

      var ID = item.dataset.id;

      var qty = _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_5___default()(_context2 = item.querySelector('.qty input').value).call(_context2);

      var tag = _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_5___default()(_context3 = item.querySelector('#wbpo_tag_text').value).call(_context3);

      var offer = _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_5___default()(_context4 = item.querySelector('#wbpo_offer_text').value).call(_context4);

      var belowOffer = _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_5___default()(_context5 = item.querySelector('#wbpo_below_offer_text').value).call(_context5);

      var aboveVariation = _babel_runtime_corejs3_core_js_stable_instance_trim__WEBPACK_IMPORTED_MODULE_5___default()(_context6 = item.querySelector('#wbpo_above_variation_text').value).call(_context6);
      ...
      products.push({
        ...
        tag: tag,
        offer: offer,
        belowOffer: belowOffer,
        aboveVariation: aboveVariation
      });
    });

    document.getElementById('wbpo_current_group').value = _babel_runtime_corejs3_core_js_stable_json_stringify__WEBPACK_IMPORTED_MODULE_6___default()(products);
  }
}
/**
 * Quantity change on Backend.
 *
 * @since 1.0.0
 */


function qtyChangeListener() {
  if (document.querySelectorAll('.wbpo-search-products .qty > input').length) {
    var _context7;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context7 = document.querySelectorAll('.wbpo-search-products .qty > input')).call(_context7, function (item) {
...
/**
 * Remove group Product item from list.
 *
 * @since 1.0.0
 */


function removeGpItem() {
  if (document.querySelector('.wbpo-search-products li .remove')) {
    var _context8;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context8 = document.querySelectorAll('.wbpo-search-products li .remove')).call(_context8, function (item) {
...
/**
 * More Info field change save value.
 *
 * @since 1.0.0
 */


function moreInfoChangeListener() {
  if (document.querySelector('.wbpo-search-products .more-info')) {
    var _context9;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context9 = document.querySelectorAll('.wbpo-search-products .wbpo-more-detail input')).call(_context9, function (input) {
...
/**
 * Show/Hide more info details.
 *
 * @param {object} e event of the listener.
 */


function showMoreInfoDetail(e) {
  var productList = e.target.closest('li');
  var moreInfoSection = productList.querySelector('.wbpo-more-detail');
...
/**
 * More info open/close fields.
 *
 * @since 1.0.0
 */


function moreInfoGpItem() {
  if (document.querySelector('#wbpo_selected .more-info')) {
    var _context10;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context10 = document.querySelectorAll('#wbpo_selected .more-info')).call(_context10, function (item) {
      item.removeEventListener('click', showMoreInfoDetail);
      item.addEventListener('click', showMoreInfoDetail);
    });
  }
}
/**
...
function WbPoSearchRequest() {
  var formData = new FormData();
  formData.append('action', 'wbpo_search_products');
  formData.append('search_input', document.querySelector('#wbpo_search_input').value);
  formData.append('current_group', document.querySelector('#wbpo_current_group').value);
  document.querySelector('#wbpo_search_input').classList.add('loading-input');
  fetch(WbPo.ajaxUrl, {
    method: 'POST',
    body: formData
  }).then(function (res) {
  ...
  .then(function (response) {
    document.querySelector('#wbpo_search_input').classList.remove('loading-input');

    if (response.data.data) {
      document.getElementById('wbpo-results').innerHTML = response.data.data;
      document.getElementById('wbpo-results').classList.remove('hidden');
    }

    if (response.success) {
      if (document.querySelectorAll('#wbpo-results li ')) {
        var _context11;

        _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context11 = document.querySelectorAll('#wbpo-results li')).call(_context11, function (item) {
          item.querySelector('.add').addEventListener('click', function (e) {
            var productItem = e.target.parentElement.parentElement;
            productItem.querySelector('.add').innerHTML = '×';
            productItem.querySelector('.add').classList.add('remove');
            productItem.querySelector('.add').classList.remove('add');
            document.querySelector('#wbpo_selected ul').append(productItem);
            document.getElementById('wbpo-results').classList.add('hidden');
            setSelectedItems();
            qtyChangeListener();
            removeGpItem();
            moreInfoGpItem();
            moreInfoChangeListener();
          });
        });
      }
    }
  })["catch"](function (err) {
    console.log("sorry,  Some issue in search ".concat(err, " ")); // eslint-disable-line
  });
}
/**
 * Hide and show qty input.
 *
 * @since 1.0.0
...
function WbPoQtyInputVisibility() {
  var _hide = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;

  if (_hide) {
    var _context12;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context12 = document.querySelectorAll('.main-content .qty')).call(_context12, function (item) {
      item.classList.remove('hidden');
    });
  } else {
    var _context13;

    _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_4___default()(_context13 = document.querySelectorAll('.main-content .qty')).call(_context13, function (item) {
...
document.addEventListener('DOMContentLoaded', function () {
  if (document.getElementById('sortlist')) {
    sortablejs__WEBPACK_IMPORTED_MODULE_8__.default.create(document.getElementById('sortlist'), {
      handle: '.move',
  ...
  qtyChangeListener(); // Qty change setSelected items.

  removeGpItem(); // Remove item from selected List.

  moreInfoGpItem(); // Open/close more info section.

  moreInfoChangeListener(); // More info fields change listner.

  var SEARCH_INPUT = document.querySelector('#wbpo_search_input');
  ...
  if (SEARCH_INPUT) {
    ...
    SEARCH_INPUT.addEventListener('keydown', function () {
      if (SEARCH_INPUT.value !== '') {
        if (WbPoTimeout !== null) {
          clearTimeout(WbPoTimeout);
        }

        WbPoTimeout = _babel_runtime_corejs3_core_js_stable_set_timeout__WEBPACK_IMPORTED_MODULE_7___default()(WbPoSearchRequest, 300);
      }
    });
  }

  if (document.getElementById('wbpo_selection_type')) {
    document.getElementById('wbpo_selection_type').addEventListener('change', function (e) {
      if (e.target.value === 'qty') {
        WbPoQtyInputVisibility();
      } else {
        WbPoQtyInputVisibility(false);
      }
    });
  }

  if (document.getElementById('wbpo_selection_type') && document.getElementById('wbpo_selection_type').value === 'qty') {
    WbPoQtyInputVisibility();
  } else {
    WbPoQtyInputVisibility(false);
  }
